...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  if (req.method === 'OPTIONS') {
    ...
    form.parse(req, async (err, fields, files) => {
      ...
      const weredaId = Array.isArray(fields.weredaId) ? fields.weredaId[0] : fields.weredaId;
      const description = Array.isArray(fields.description) ? fields.description[0] : fields.description;
      const scheduleId = Array.isArray(fields.scheduleId) ? fields.scheduleId[0] : fields.scheduleId;
      console.log(fields)
      console.log(files)

      if (!name || !description || !scheduleId || !weredaId) {
        return res.status(400).json({ error: 'Name, description, weredaId and scheduleId are required' });
      }
      ...
      try {
        ...
        const newReport = await prisma.report.create({
          ...
          audio: audioPath,
          reportedByWeredaId: Number(weredaId),
        ...
        allWana.map(async (wana)=>{
          ...
          <div style="font-family: Arial, sans-serif; max-width: 600px; margin: auto; border: 1px solid #ddd; border-radius: 8px; overflow: hidden;">
            <div style="background-color: #897848; padding: 20px; text-align: center; color: white;">
              ...
              <p>Wereda ${firstName} ${lastName} as reported for the meeting ${scheduledMetting.title} held between 
        ...
        await prisma.notification.create({
          data: {
            message: `Wereda ${firstName} ${lastName} as reported for the meeting ${scheduledMetting.title} held between 
