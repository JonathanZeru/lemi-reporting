...
import { WanaRegistrationFormData, wanaRegistrationSchema } from '@/schemas/wanaRegistrationSchema'

const WanaRegistration = () => {
  ...
  const {
    ...
    formState: { errors },
    watch,
  } = useForm<WanaRegistrationFormData>({
    resolver: zodResolver(wanaRegistrationSchema),
    defaultValues: {
      firstName: '',
      lastName: '',
      email: '',
  ...
  const onSubmit = async (data: WanaRegistrationFormData) => {
    setLoading(true)

    try {
      const response = await axios.post(`${apiURL}api/wana/register`, data)
...
export default WanaRegistration
