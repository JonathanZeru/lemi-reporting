...
const fetchReport = async () => {
  try {
    const response = await axios.get<HiwasNotification>(`${apiURL}api/meseretawi/notification?hiwasId=${user?.id}&notificationId=${id}`)
    if (response.data) {
  ...
  useEffect(() => {
    const fetchReport = async () => {
        try {
            console.log(`${apiURL}api/meseretawi/notification?meseretawiId=${user?.id}&notificationId=${id}`)
          const response = await axios.get<HiwasNotification>(`${apiURL}api/meseretawi/notification?meseretawiId=${user?.id}&notificationId=${id}`)
          if (response.data) {
            setNotification(response.data)
          } else {
            setError('No Schedule found')
          ...
          <CardContent>
            <div className="grid grid-cols-2 gap-4 print:grid-cols-1">
              ...
              <div>
                ...
                <p><strong>ሁኔታ:</strong> <Badge>{reportStatus}</Badge> እና it is a <Badge>{notification.schedule.status}</Badge> መርሐግብር.</p>
              </div>
            :
            <></>}
            ...
            <div>
              ...
              <Link href={`/dashboard/report-detail/${notification.report.id}`}>
              <Button>
              ተጨማሪ ዝርዝር ይመልከቱ
              </Button>
              </Link>
...
export default MeseretawiTaskDetail
